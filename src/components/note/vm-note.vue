<template>
  <div class="vm-image-list">
    ...
    <Row class="image-list" :gutter="15">
      <div class="panels">
        <Row :gutter="15">
          <Col span="12" v-for="item in dataShow" :key="item.id">
            <VmNoteCard :editable="true" :note="item" :title="item.title" :desc="item.content" :bookId="item.bookId" :editUrl="item.editUrl" @delete-ok=" deleteOk(item) " @edit-ok=" editOk(item) "></VmNoteCard>
          </Col>
        </Row>
      </div>
    </Row>
...
<script>
  import VmNoteCard from '@/components/note/vm-note-card'
